...
object ZConfig {
  ...
  private final val APP_MODE = "mode"

  private val modeFromProp = System.getProperty(APP_MODE)
  private val modeFromEnv = System.getenv(APP_MODE)
  ...
  def loadFile(file: String) = ConfigFactory.parseFile(new File(file))
  ...
  implicit class ZConfigLike(config: Config) {
