...
class MoveESTool extends Test {
 ...
 // private val client = createClient(clusterName = "xed_live_cluster", servers = Seq("34.87.143.227:9300"))
  ...
  test("Migrate BOT SRS Model to Kiki SRS") {

    val srcClient = client

    val backupIndexConfs = Seq(
      ( "srs", Seq("course", "bot_srs"))
    )
    ...
    backupIndexConfs.foreach({
      case (index, types) =>
        ...
        searchScroll(srcClient, index, Option(types), searchHits => backupSRSModel(
          srcClient,
  ...
  test("Duplicate SRS index to a new index") {
    ...
    backupIndexConfs.foreach({
      case (index, types) =>
        ...
        searchScroll(srcClient, index, Option(types), searchHits => backup(
          srcClient,
          newIndexMap.get(index).get,
          searchHits,
          successCounter,
  ...
  test("Migrate SRS Model to Kiki SRS") {
